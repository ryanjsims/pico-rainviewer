...
int32_t MC_23LCV1024::read(uint32_t addr, std::span<uint8_t> dst) {
    ...
    uint8_t buf[4] = {READ_OP, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF};
    cs_select();
    spi_write_blocking(spi0, buf, 4);
    spi_read_blocking(spi0, 0x00, dst.data(), dst.size());
    cs_deselect();
}
...
int32_t MC_23LCV1024::write(uint32_t addr, std::span<uint8_t> src) {
    ...
    uint8_t buf[4] = {WRITE_OP, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF, addr & 0xFF};
    cs_select();
    spi_write_blocking(spi0, buf, 4);
    spi_write_blocking(spi0, src.data(), src.size());
    cs_deselect();
}
